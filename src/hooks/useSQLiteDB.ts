...
const DB_NAME = 'longevai_financial_data';
...
export const useSQLiteDB = () => {
  ...
  const saveFinancialData = useCallback(async (financialData: FinancialData) => {
    ...
    try {
      ...
      db.run(`
        INSERT INTO financial_summary (total_revenue, total_expenses, net_profit, current_balance, outstanding_receivables)
        VALUES (?, ?, ?, ?, ?)
      `, [
        ...
        financialData.outstandingReceivables
      ]);

      saveDatabase();
      console.log('Financial data saved to SQLite');
    } catch (error) {
